...
#
# ⚠ 正式比赛还有两件本脚本不做的事（见 docs/4-客户端侧.md）：
#   1) anticheatd 启动 + session ticket 校验（P3）
#   2) L0 启动参数白名单（本脚本只做正向拼参数，拦截逻辑在反作弊）
set -euo pipefail
...
echo "[launcher] mode=$MODE callsign=$CALLSIGN server=$CTF_SERVER"
exec "$FG_BIN" "${ARGS[@]}"
